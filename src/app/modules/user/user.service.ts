...
const updateUser = async ( userId: string, payload: Partial<IUser>, decodedToken: JwtPayload ): Promise<IUser> => {
  ...
  if (payload.role) {
    ...
    if (requesterRole === Role.ADMIN) {
      if ([Role.ADMIN, Role.SUPER_ADMIN].includes(payload.role)) {
        throw new AppError(httpStatus.FORBIDDEN, 'ADMIN cannot assign ADMIN or SUPER_ADMIN roles');
      }
    } 
    ...
    if (requesterRole === Role.SUPER_ADMIN && payload.role === Role.SUPER_ADMIN) {
      throw new AppError(httpStatus.FORBIDDEN, 'SUPER_ADMIN cannot assign SUPER_ADMIN role');
    } 
    ...
    if (payload.role === Role.ADMIN && requesterRole !== Role.SUPER_ADMIN) {
      throw new AppError(httpStatus.FORBIDDEN, 'Only SUPER_ADMIN can assign ADMIN role');
    } 
    ...
    if (requesterRole === Role.SUPER_ADMIN) {
      allowedRolesToAssign.push(Role.ADMIN); // SUPER_ADMIN এর জন্য ADMIN ও ALLOWED
    } 
    if (!allowedRolesToAssign.includes(payload.role)) {
      throw new AppError(httpStatus.FORBIDDEN, 'Invalid role assignment');
    }
  } 
